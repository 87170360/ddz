...
//	vtime = hlddz.main_rc[index]->get_value_as_int("vtime");
	...
	// cfc add 20140321
	forbidden_card = hlddz.main_rc[index]->get_value_as_int("forbidden_card");
	change_card = hlddz.main_rc[index]->get_value_as_int("change_card");
	double_card = hlddz.main_rc[index]->get_value_as_int("double_card");
...
int Player::incr_coin(int value)
{
	...
	int ret = hlddz.main_rc[index]->command("hincrby hu:%d coin %d", uid, value);
	if (ret < 0) {
		xt_log.error("incr coin error.\n");
		return -1;
	}
	xt_log.info("incr coin uid[%d] old[%d] new[%d].\n", uid, coin, hlddz.main_rc[index]->reply->integer);
	coin = hlddz.main_rc[index]->reply->integer; //update the coin.
...
int Player::incr_exp(int value)
{
	...
	int ret;
	ret = hlddz.main_rc[index]->command("hincrby hu:%d exp %d", uid, value);
	if (ret < 0) {
		xt_log.error("incr exp error.\n");
		return -1;
	}
	xt_log.info("incr exp uid[%d] old[%d] new[%d].\n", uid, exp, hlddz.main_rc[index]->reply->integer);
	exp = hlddz.main_rc[index]->reply->integer; //update the exp.
...
int Player::incr_rmb(int value)
{
	...
	int ret;
	ret = hlddz.main_rc[index]->command("hincrby hu:%d rmb %d", uid, value);
	if (ret < 0) {
		xt_log.error("incr rmb error.\n");
		return -1;
	}
	xt_log.info("incr rmb uid[%d] old[%d] new[%d].\n", uid, rmb, hlddz.main_rc[index]->reply->integer);
	rmb = hlddz.main_rc[index]->reply->integer; //update the rmb.
...
int Player::incr_total_board(int vid, int value)
{
	...
	int ret;
	ret = hlddz.main_rc[index]->command("hincrby hu:%d total_board %d", uid, value);
	if (ret < 0) {
		xt_log.error("incr total board error.\n");
		return -1;
	}
	total_board = hlddz.main_rc[index]->reply->integer; //update the total_board.
...
int Player::incr_total_win(int vid, int value)
{
	...
	int ret;
	ret = hlddz.main_rc[index]->command("hincrby hu:%d total_win %d", uid, value);
	if (ret < 0) {
		xt_log.error("incr total win error.\n");
		return -1;
	}
	total_win = hlddz.main_rc[index]->reply->integer; //update the total_win.
...
int Player::incr_pcount(int value)
{
	...
	int ret;
	ret = hlddz.main_rc[index]->command("hincrby hu:%d play_count %d", uid, value);
	if (ret < 0) {
		xt_log.error("incr pcount error.\n");
		return -1;
	}
	pcount = hlddz.main_rc[index]->reply->integer; //update the total_board.
...
int Player::incr_achievement_count(int my_vid, int type, int value)
{
	...
	switch (my_vid) {
		case 1:
			ret = hlddz.main_rc[index]->command("hincrby st:%d room1 %d", uid, value);
			break;
		case 2:
			ret = hlddz.main_rc[index]->command("hincrby st:%d room2 %d", uid, value);
			break;
		case 3:
			ret = hlddz.main_rc[index]->command("hincrby st:%d room3 %d", uid, value);
			break;
		case 4:
			ret = hlddz.main_rc[index]->command("hincrby st:%d room4 %d", uid, value);
			break;
		case 5:
			ret = hlddz.main_rc[index]->command("hincrby st:%d room5 %d", uid, value);
			break;
		case 6:
			if (type == 0)
				ret = hlddz.main_rc[index]->command("hincrby st:%d playoffs_win %d", uid, value);
			ret = hlddz.main_rc[index]->command("hincrby st:%d playoffs_count %d", uid, value);
			break;
		case 7:
			if (type == 0)
				ret = hlddz.main_rc[index]->command("hincrby st:%d winner_win %d", uid, value);
			ret = hlddz.main_rc[index]->command("hincrby st:%d winner_count %d", uid, value);
			break;
		case 8:
			ret = 0;
			break;
		case 901:
			if (type == 0)
				ret = hlddz.main_rc[index]->command("hincrby st:%d daily_props_win %d", uid, value);
			ret = hlddz.main_rc[index]->command("hincrby st:%d daily_props_play %d", uid, value);
			ret = hlddz.main_rc[index]->command("hincrby st:%d room201 %d", uid, value);
			break;
		case 902:
			if (type == 0)
				ret = hlddz.main_rc[index]->command("hincrby st:%d daily_props_win %d", uid, value);
			ret = hlddz.main_rc[index]->command("hincrby st:%d daily_props_play %d", uid, value);
			ret = hlddz.main_rc[index]->command("hincrby st:%d room202 %d", uid, value);
			break;
		case 903:
			if (type == 0)
				ret = hlddz.main_rc[index]->command("hincrby st:%d daily_props_win %d", uid, value);
			ret = hlddz.main_rc[index]->command("hincrby st:%d daily_props_play %d", uid, value);
			ret = hlddz.main_rc[index]->command("hincrby st:%d room203 %d", uid, value);
			break;
		default:
			ret = -1;
			break;
	...
	if (type == 0)
		ret = hlddz.main_rc[index]->command("hincrby st:%d daily_win_count %d", uid, value);
	ret = hlddz.main_rc[index]->command("hincrby st:%d daily_play_count %d", uid, value);
	if (ret < 0) {
...
int Player::incr_achievement_card(int type, int value)
{
	...
	switch (type) {
		case 1:
			ret = hlddz.main_rc[index]->command("hincrby st:%d game_s2 %d", uid, value);
			break;
		case 2:
			ret = hlddz.main_rc[index]->command("hincrby st:%d game_s3 %d", uid, value);
			break;
		default:
			break;
	}
...
{
	int ret;
	ret = hlddz.main_rc[index]->command("hincrby hu:%d forbidden_card -%d", uid, value);
	if (ret < 0) {
		xt_log.error("incr forbidden_card error.\n");
		return -1;
	}
	xt_log.info("incr change_card uid[%d] old[%d] new[%d].\n", uid, forbidden_card, hlddz.main_rc[index]->reply->integer);
	forbidden_card = hlddz.main_rc[index]->reply->integer;
...
int Player::incr_change_card(int value)
{
	...
	int ret;
	ret = hlddz.main_rc[index]->command("hincrby hu:%d change_card -%d", uid, value);
	if (ret < 0) {
		xt_log.error("incr change_card error.\n");
		return -1;
	}
	xt_log.info("incr change_card uid[%d] old[%d] new[%d].\n", uid, change_card, hlddz.main_rc[index]->reply->integer);
	change_card = hlddz.main_rc[index]->reply->integer;
...
{
	int ret;
	ret = hlddz.main_rc[index]->command("hincrby hu:%d double_card -%d", uid, value);
	if (ret < 0) {
		xt_log.error("incr double_card error.\n");
		return -1;
	}
	xt_log.info("incr change_card uid[%d] old[%d] new[%d].\n", uid, double_card, hlddz.main_rc[index]->reply->integer);
	double_card = hlddz.main_rc[index]->reply->integer;
