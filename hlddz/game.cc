...
void dump_game_info(char *tag)
{
	...
	i += sprintf(buf + i, "[four_tables][%lu]\n", ddz.game->four_tables.size());
	for (table_it = ddz.game->four_tables.begin(); table_it != ddz.game->four_tables.end(); table_it++)
	{
		Table *table = table_it->second;
		i += sprintf(buf + i, "Tid[%d]state[%d] ", table_it->first, table->state);
		for (player_it = table->players.begin(); player_it != table->players.end(); player_it++)
	...
	i += sprintf(buf + i, "[three_tables][%lu]\n", ddz.game->three_tables.size());
	for (table_it = ddz.game->three_tables.begin(); table_it != ddz.game->three_tables.end(); table_it++)
	{
		Table *table = table_it->second;
		i += sprintf(buf + i, "Tid[%d]state[%d] ", table_it->first, table->state);
		for (player_it = table->players.begin(); player_it != table->players.end(); player_it++)
	...
	i += sprintf(buf + i, "[two_tables][%lu]\n", ddz.game->two_tables.size());
	for (table_it = ddz.game->two_tables.begin(); table_it != ddz.game->two_tables.end(); table_it++)
	{
		Table *table = table_it->second;
		i += sprintf(buf + i, "Tid[%d]state[%d] ", table_it->first, table->state);
		for (player_it = table->players.begin(); player_it != table->players.end(); player_it++)
	...
	i += sprintf(buf + i, "[one_tables][%lu]\n", ddz.game->one_tables.size());
	for (table_it = ddz.game->one_tables.begin(); table_it != ddz.game->one_tables.end(); table_it++)
	{
		Table *table = table_it->second;
		i += sprintf(buf + i, "Tid[%d]state[%d] ", table_it->first, table->state);
		for (player_it = table->players.begin(); player_it != table->players.end(); player_it++)
	...
	i += sprintf(buf + i, "[fd_client][%lu]\n", ddz.game->fd_client.size());
	for (client_it = ddz.game->fd_client.begin(); client_it != ddz.game->fd_client.end(); client_it++)
	{
		i += sprintf(buf + i, "fd[%d] ", client_it->first);
	}
	i += sprintf(buf + i, "\n");

	i += sprintf(buf + i, "[offline_players][%lu]\n", ddz.game->offline_players.size());
	for (player_it = ddz.game->offline_players.begin(); player_it != ddz.game->offline_players.end(); player_it++)
	{
		i += sprintf(buf + i, "uid[%d] ", player_it->first);
	}
	i += sprintf(buf + i, "\n");

	i += sprintf(buf + i, "[online_players][%lu]\n", ddz.game->online_players.size());
	for (player_it = ddz.game->online_players.begin(); player_it != ddz.game->online_players.end(); player_it++)
	{
		i += sprintf(buf + i, "uid[%d] ", player_it->first);
	}
	i += sprintf(buf + i, "\n");
...
int Game::init_table()
{
	int vid = ddz.conf["tables"]["vid"].asInt();
	int zid = ddz.conf["tables"]["zid"].asInt();
	int type = ddz.conf["tables"]["table_type"].asInt();
	int min_money = ddz.conf["tables"]["min_money"].asInt();
	int max_money = ddz.conf["tables"]["max_money"].asInt();
	int base_money = ddz.conf["tables"]["base_money"].asInt();
	int min_round = ddz.conf["tables"]["min_round"].asInt();
	int max_round = ddz.conf["tables"]["max_round"].asInt();
	float fee = atof(ddz.conf["tables"]["fee"].asString().c_str());
	int lose_exp = ddz.conf["tables"]["lose_exp"].asInt();
	int win_exp = ddz.conf["tables"]["win_exp"].asInt();
	...
	for (int i = ddz.conf["tables"]["begin"].asInt(); i < ddz.conf["tables"]["end"].asInt(); i++)
	{
...
int Game::init_accept()
{
	xt_log.info("Listening on %s:%d\n",
			ddz.conf["game"]["host"].asString().c_str(),
			ddz.conf["game"]["port"].asInt());
	...
	addr.sin_port = htons(ddz.conf["game"]["port"].asInt());
	addr.sin_addr.s_addr = inet_addr(ddz.conf["game"]["host"].asString().c_str());
	...
	ev_io_start(ddz.loop, &_ev_accept);
...
#if 1
	int i = client->uid % ddz.main_size;
	int ret = ddz.main_rc[i]->command(" hget u:%d skey", client->uid);
	if (ret < 0) {
		xt_log.error("check skey error, because get player infomation error.\n");
		return -1;
	}

	xt_log.debug("skey [%s] [%s]\n", client->skey.c_str(), ddz.main_rc[i]->reply->str);
	if (ddz.main_rc[i]->reply->str && client->skey.compare(ddz.main_rc[i]->reply->str) != 0) {
...
int Game::add_player(Client *client)
{
	...
	if(ddz.cache_rc->command("hset gameinfo %d %d",ddz.conf["game"]["port"].asInt(),fd_client.size())<0)
	{
...
int Game::del_player(Player *player)
{
	...
	if(ddz.cache_rc->command("hset gameinfo %d %d",ddz.conf["game"]["port"].asInt(),fd_client.size()))
	{
		xt_log.error("cache redis error(set gameinfo");
	}
