...
XtRobotClient::~XtRobotClient()
{
	if(m_serverfd!=-1)
	{
		...
		ev_timer_stop(m_evloop,&m_evJoinGameTimer);
		close(m_serverfd);
	}
...
void XtRobotClient::onJoinGame(struct ev_loop* loop,struct ev_timer* w,int events)
{
	ev_timer_stop(loop,w);
	XtRobotClient* self = (XtRobotClient*) w->data;
	self->sendJoinGamePacket();
}
...
int XtRobotClient::onReciveCmd(Jpacket& data)
{
	...
	case SERVER_LOGIN_SUCC_BC:
		printf("广播登录成功\n");
		break;

	case SERVER_GAME_WAIT_BC://等待
		printf("广播游戏等待(清理房间)\n");
		break;

	case SERVER_GAME_READY_BC://准备
		printf("广播游戏准备\n");
		break;

	case SERVER_MAKE_BANKER_BC://定庄
		printf("广播随机庄家,游戏开始\n");
		break;

	case SERVER_SEND_CARD_UC:
		printf("给玩家发牌\n");
		handleSendCard(val);
		break;

	case SERVER_GROUP_CARD_UC:
		printf("组牌应答\n");			
		break;

	case SERVER_GROUP_CARD_BC:
		printf("广播玩家组牌\n");
		break;

	case SERVER_COMPARE_CARD_BC:
		printf("广播比牌，游戏结束\n");
		break;

	case SERVER_JOIN_GAME_UC:
		printf("加入游戏应答\n");
		break;
		
	case SERVER_JOIN_GAME_BC:
		printf("加入游戏广播\n");
		break;

	case SERVER_KICK_PLAYER_UC:
		printf("踢人应答\n");
		break;

	case SERVER_KICK_PLAYER_BC:
		printf("踢人广播\n");
		break;

	case SERVER_DISMISS_ROOM_UC:
		printf("解散房间应答\n");
		break;	

	case SERVER_DISMISS_ROOM_BC:
		printf("解散房间广播\n");
		break;	
	}
...
// 开始游戏
void XtRobotClient::handleJoinGame(Json::Value& data)
{
...
int XtRobotClient::connectToServer(const char* ip,int port,int uid)
{
	...
	ev_timer_init(&m_evJoinGameTimer,XtRobotClient::onJoinGame,4,0);   
...
void XtRobotClient::sendLoginPackage()
{
	...
	//+++私密场
    data.val["room_id"]       = 2;         
    data.val["room_password"] = "111";
    data.val["game_rule"]     = 1;
	data.end();
...
void XtRobotClient::sendChoiceCardPacket()
{
	printf("上行玩家组牌\n");
	Jpacket data;
	data.val["cmd"] = CLIENT_GROUP_CARD_REQ;
...
// 加入游戏
void XtRobotClient::sendJoinGamePacket()
{
	printf("上行加入游戏\n");
	Jpacket data;
	data.val["cmd"] = CLIENT_JOIN_GAME_REQ;
	data.end();
...
//加入游戏
void XtRobotClient::doJoinGame()    
{
	ev_timer_stop(m_evloop, &m_evJoinGameTimer);
	ev_timer_set(&m_evJoinGameTimer, rand()%5+2,0);   ///随机组牌时间
	ev_timer_start(m_evloop, &m_evJoinGameTimer);
}
