#ifndef _NORMAL_TABLE_H_
#define _NORMAL_TABLE_H_
...
{
	NORMAL_READY = 0,     ///准备
	NORMAL_BANKER,        ///选庄（游戏开始）
	NORMAL_SEND_CARD,     ///发牌
	NORMAL_COMPARE,       ///比牌（游戏结束）
	NORMAL_WAIT,          ///等待
} EnNormalGameState;
...
public:
	...
	int                    lose_exp;     ///输的经验值
	int                    win_exp;      ///赢的经验值
	EnNormalGameState	   game_state;   // game state(即游戏场景)
	...
	NormalSeat                   seats[MAX_CHAIR_COUNT]; // 坐位数组(一张桌子5个位置)
...
public:
	...
	// 房主踢人
	int kickPlayer(Player *player);

	// 解散房间
	int dismissRoom(Player *player);
	...
	//组牌
	void group_cards(NormalSeat &seat);
