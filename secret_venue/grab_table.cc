...
///定义各阶段时间
const int GRAB_READY_TIME     = 5;    // 准备时间
const int GRAB_FIRST_SEND_TIME= 10;   // 第一次发牌时间（此阶段用户抢庄加倍）
const int GRAB_BANKER_TIME    = 10;   // 广播庄家时间（此阶段闲家下注加倍）
const int GRAB_SECOND_SEND_TIME= 15;  // 第二次发牌时间（此阶段用户组牌）
const int GRAB_COMPARE_TIME   = 5;    // 比牌时间
...
GrabTable::GrabTable() :
  ready_timer_stamp(GRAB_READY_TIME),            // 1准备时间（匹配人）
  first_card_timer_stamp(GRAB_FIRST_SEND_TIME),  // 2首次发4张牌(用户抢庄)
  banker_timer_stamp(GRAB_BANKER_TIME),          // 3选庄时间
  second_card_timer_stamp(GRAB_SECOND_SEND_TIME),// 4二次发牌 
  compare_timer_stamp(GRAB_COMPARE_TIME),        // 5比牌时间（即游戏结束，比牌结算）
  robot_timer_stamp(6)                      // 机器人时间
{
 	// 1准备（这个时间可以有玩家进来或者离开）
    ready_timer.data = this;
    ev_timer_init(&ready_timer, GrabTable::ready_timer_cb, ready_timer_stamp, ready_timer_stamp);

	// 2首次发牌
	first_card_timer.data = this;
	ev_timer_init(&first_card_timer, GrabTable::first_card_timer_cb, first_card_timer_stamp, first_card_timer_stamp);

	// 3广播庄家
	banker_timer.data = this;
	ev_timer_init(&banker_timer, GrabTable::banker_timer_cb, banker_timer_stamp, banker_timer_stamp);

	// 4第二次发牌
	second_card_timer.data = this;
	ev_timer_init(&second_card_timer, GrabTable::second_card_timer_cb, second_card_timer_stamp, second_card_timer_stamp);

	///5比牌
    compare_timer.data = this;
    ev_timer_init(&compare_timer, GrabTable::compare_timer_cb, compare_timer_stamp, compare_timer_stamp);


    robot_timer.data = this;
    ev_timer_init(&robot_timer, GrabTable::robot_timer_cb, robot_timer_stamp, robot_timer_stamp);
...
GrabTable::~GrabTable()
{
	...
	ev_timer_stop(gBull.loop, &first_card_timer);
	ev_timer_stop(gBull.loop, &banker_timer);
	ev_timer_stop(gBull.loop, &second_card_timer);
...
/**
   玩家登录进来(满5人就不能再进了)
**/
int GrabTable::handler_login(Player *player)
{
	if (playersMap.find(player->uid) == playersMap.end()) 
	{
		...
		xt_log.info("Func[%s] Login GrabTable Succeed, uid[%d] money[%d] cur_players[%d] tid[%d] game_state[%d].\n", 
...
///广播游戏准备
int GrabTable::send_game_ready_bc()
{
	Jpacket packet;
...
/**坐下,分配空闲坐位
   ...
   return: 坐位号
*/
int GrabTable::sit_down(Player *player)
{
	//空坐位列表
	std::vector<int> tmp;     
    for (int i = 0; i < MAX_CHAIR_COUNT; i++)
...
///下行登录成功应答
int GrabTable::send_login_succ_uc(Player *player)
{
	Jpacket packet;
	...
	//每个阶段时间
	packet.val["ready_time"]       = GRAB_READY_TIME;  
	packet.val["first_send_time"]  = GRAB_FIRST_SEND_TIME;	
	packet.val["banker_time"]      = GRAB_BANKER_TIME;
	packet.val["second_send_time"] = GRAB_SECOND_SEND_TIME;	
	packet.val["compare_time"]     = GRAB_COMPARE_TIME;
...
///登录成功广播（“我”的信息广播出去）
int GrabTable::send_login_succ_bc(Player *player)
{
	GrabSeat &seat = seats[player->seatid];
...
//下行房间数据
int GrabTable::send_table_info_uc(Player *player)
{
	Jpacket packet;
	...
	case READY:
		state_left_time = GRAB_READY_TIME - (time(0) - start_ready_time);
		break;

	case FIRST_CARD:
		state_left_time = GRAB_FIRST_SEND_TIME -(time(0) - start_first_card_time);
		break;

	case BANKER:
		state_left_time = GRAB_BANKER_TIME -(time(0) - start_banker_time);
		break;

	case SECOND_CARD:
		state_left_time = GRAB_SECOND_SEND_TIME -(time(0) - start_second_card_time);
		break;

	case COMPARE:
		state_left_time = GRAB_COMPARE_TIME - (time(0) - start_compare_time);
		break;
	default:
		state_left_time = 0;
		break;
...
//“准备”时间结束，进入“首次”发牌
void GrabTable::ready_timer_cb(struct ev_loop *loop, struct ev_timer *w, int revents)
{
	GrabTable *table = (GrabTable*)w->data;
...
{	
	...
	ev_timer_again(gBull.loop, &first_card_timer);
...
///广播选庄结果
int GrabTable::send_make_banker_bc()
{
	Jpacket packet;
...
///"选庄"时间完毕， 游戏进入“二次发牌”
void GrabTable::banker_timer_cb(struct ev_loop *loop, struct ev_timer *w, int revents)
{
	...
	ev_timer_again(gBull.loop, &table->second_card_timer);
}

//“准备”时间结束，首次发牌
void GrabTable::first_send_card()
{
	//1先组好牌
...
//二次发牌
void GrabTable::second_send_card()
{
	// 二次发牌开始时间
...
// 组牌
void GrabTable::update_best_board(GrabSeat &seat)
{
...
///"首次发牌"时间结束， 进入选庄
void GrabTable::first_card_timer_cb(struct ev_loop *loop, struct ev_timer *w, int revents)
{
	GrabTable *table = (GrabTable*) w->data;
	ev_timer_stop(gBull.loop, &table->first_card_timer);
	...
	ev_timer_again(gBull.loop, &table->banker_timer);	
...
// “二次发牌”时间结束, 进入比牌
void GrabTable::second_card_timer_cb(struct ev_loop *loop, struct ev_timer *w, int revents)
{
	GrabTable *table = (GrabTable*) w->data;
	ev_timer_stop(gBull.loop, &table->second_card_timer);
...
// 校验“抢庄”是否满足条件
bool GrabTable::checkDoubleGrab(Player *player, int doubleGrab)
{
...
// 校验“下注”是否满足条件
bool GrabTable::checkDoubleBet(Player *player, int doubleBet)
{
...
/**
   玩家抢庄
*/
int GrabTable::handler_grab_banker(Player *player)
{
	int errCode = 0;   //错误码
...
/**
   闲家下注
*/
int GrabTable::handler_double_bet(Player *player)
{
	int errCode = 0;   //错误码
	...
	if (game_state == BANKER)  
	{
		...
		if (isAllDoubleBet())
		{
			...
			ev_timer_again(gBull.loop, &second_card_timer);	
		}
	}
...
/**
   用户上行组牌(校验牌、记录组牌时间)
*/
int GrabTable::handler_group_card(Player *player)
{
	int errCode = 0;   //错误码
...
bool GrabTable::isAllPlayerChoiced() 
{
	for (int i = 0; i < MAX_CHAIR_COUNT; i++)
	{
		if (seats[i].status >= STATUS_IN_GAME 
...
///"比牌"时间结束, 游戏结束，再次进入等待
void GrabTable::compare_timer_cb(struct ev_loop *loop, struct ev_timer *w, int revents)
{
...
///---处理登出
int GrabTable::handler_logout(Player *player)
{	
...
///---发聊天
int GrabTable::handler_chat(Player *player)
{
	Json::Value &val = player->client->packet.tojson();
...
///发表情
int GrabTable::handler_face(Player *player)
{
	Json::Value &val = player->client->packet.tojson();
...
///机器人定时器回调
void GrabTable::robot_timer_cb(struct ev_loop *loop, struct ev_timer *w, int revents)
{
	GrabTable *table = (GrabTable*) w->data;
...
void GrabTable::lose_exp_update(Player *player)
{
	player->incr_exp(lose_exp);
}

void GrabTable::win_exp_update(Player *player)
{
	player->incr_exp(win_exp);
}

///把单个玩家信息广播出去
int GrabTable::handler_info(Player *player)
{
	int ret = 0;
...
///---处理互动表情
int GrabTable::handler_interaction_emotion(Player *player)
{
	Json::Value &val = player->client->packet.tojson();
...
///站起，则清空坐位
void GrabTable::stand_up(Player *player)
{
...
///删除玩家
int GrabTable::del_player(Player *player)
{
...
int GrabTable::broadcast(Player *p, const std::string &packet)
{
	Player *player;
	std::map<int, Player*>::iterator it;
	for (it = playersMap.begin(); it != playersMap.end(); it++)
...
///单播
int GrabTable::unicast(Player *p, const std::string &packet)
{
...
///[starr, end]之间的随机数
int GrabTable::random(int start, int end)
{
...
///vector转JSON数组
//void GrabTable::vector_to_json_array(std::vector<XtCard> &cards, Jpacket &packet, string key)
//{
...
//
/////map转JSON数组
//void GrabTable::map_to_json_array(std::map<int, XtCard> &cards, Jpacket &packet, string key)
//{
...
//
/////JSON数组转vector
//void GrabTable::json_array_to_vector(std::vector<XtCard> &cards, Jpacket &packet, string key)
//{
