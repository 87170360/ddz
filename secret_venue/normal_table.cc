  ...
  robot_timer_stamp(6)               // 机器人时间
{
    ...
    ev_timer_init(&ready_timer, NormalTable::ready_timer_cb, ready_timer_stamp, ready_timer_stamp);

	///2随机庄家
	banker_timer.data = this;
	ev_timer_init(&banker_timer, NormalTable::banker_timer_cb, banker_timer_stamp, banker_timer_stamp);

	///3发牌
	send_card_timer.data = this;
	ev_timer_init(&send_card_timer, NormalTable::send_card_timer_cb, send_card_timer_stamp, send_card_timer_stamp);

	///4比牌
    compare_timer.data = this;
    ev_timer_init(&compare_timer, NormalTable::compare_timer_cb, compare_timer_stamp, compare_timer_stamp);


    robot_timer.data = this;
    ev_timer_init(&robot_timer, NormalTable::robot_timer_cb, robot_timer_stamp, robot_timer_stamp);
...
/**
   玩家登录进来(满5人就不能再进了)
**/
int NormalTable::handler_login(Player *player)
{
	if (playersMap.find(player->uid) == playersMap.end()) {
		...
		xt_log.info("Func[%s] Login NormalTable Succeed, uid[%d] money[%d] cur_players[%d] tid[%d] game_state[%d].\n", 
			        __FUNCTION__, player->uid, player->money, cur_players, tid, game_state);

		//当前状态NORMAL_WAIT状态，且玩家数>=2人，游戏状态为“准备”
		if((NORMAL_WAIT == game_state) && (cur_players >= 2))   
		{
			xt_log.info("Func[%s] *** 游戏准备 NORMAL_READY.\n", __FUNCTION__);

			//游戏状态为"准备"
			game_state = NORMAL_READY;
...
///广播游戏准备
int NormalTable::send_game_ready_bc()
{
	Jpacket packet;
	packet.val["cmd"] = SERVER_GAME_READY_BC;
	packet.val["state"] = NORMAL_READY;
	packet.end();
...
/**坐下,分配空闲坐位
   ...
   return: 坐位号
*/
int NormalTable::sit_down(Player *player)
{
	//空坐位列表
	std::vector<int> tmp;     
    for (int i = 0; i < MAX_CHAIR_COUNT; i++)
...
///下行登录成功应答
int NormalTable::send_login_succ_uc(Player *player)
{
	Jpacket packet;
...
///下行登录成功广播（“我”的信息广播出去）
int NormalTable::send_login_succ_bc(Player *player)
{
	NormalSeat &seat = seats[player->seatid];
...
//下行房间数据
int NormalTable::send_table_info_uc(Player *player)
{
	Jpacket packet;
	...
	{
		Player *p = it->second;
		NormalSeat &seat = seats[p->seatid];
...
//“准备”时间结束，进入选庄
void NormalTable::ready_timer_cb(struct ev_loop *loop, struct ev_timer *w, int revents)
{
	NormalTable *table = (NormalTable*)w->data;
	...
	{
		//人数不够，游戏继续等待		
		xt_log.info("Func[%s] *** 人数不够，游戏进入 NORMAL_WAIT.\n", __FUNCTION__);
		table->game_state = NORMAL_WAIT; 

		//广播游戏等待
		Jpacket packet;
		packet.val["cmd"]   = SERVER_GAME_WAIT_BC;
		packet.val["state"] = NORMAL_WAIT;
		packet.end();
...
//设置玩家状态为“游戏中” 
int NormalTable::setPlayStatus(int bankSeatid)
{	
...
{	
	...
	if (dealer < 0)
	{
		...
		//游戏状态“选庄”
		game_state = NORMAL_BANKER;
		ev_timer_again(gBull.loop, &banker_timer);
		start_banker_time = time(0);  //选庄开始时刻	
	} 
	else 
	{
		xt_log.debug("Func[%s] *** 有庄家，直接NORMAL_SEND_CARD dealer[%d]\n", __FUNCTION__, dealer);
        game_state = NORMAL_SEND_CARD;
		//有庄家，直接发牌
		send_card();
		///进入发牌环节
...
///广播选庄结果
int NormalTable::send_make_banker_bc()
{
	Jpacket packet;
	packet.val["cmd"] = SERVER_MAKE_BANKER_BC;
	packet.val["state"]  = NORMAL_BANKER;	
...
///"选庄"时间完毕， 发牌
void NormalTable::banker_timer_cb(struct ev_loop *loop, struct ev_timer *w, int revents)
{
	xt_log.debug("Func[%s] *** 游戏发牌 NORMAL_SEND_CARD.\n", __FUNCTION__);
	...
	///进入发牌环节
	table->game_state = NORMAL_SEND_CARD;
...
void NormalTable::send_card()
{
	...
	for (it = playersMap.begin(); it != playersMap.end(); it++) 
	{
		...
		packet.val["state"] = NORMAL_SEND_CARD;
		//牌数据
...
///组牌
void NormalTable::group_cards(NormalSeat &seat)
{
...
///"发牌"时间结束， 进入比牌
void NormalTable::send_card_timer_cb(struct ev_loop *loop, struct ev_timer *w, int revents)
{
	NormalTable *table = (NormalTable*) w->data;
	ev_timer_stop(gBull.loop, &table->send_card_timer);

	xt_log.debug("Func[%s] *** 游戏比牌 NORMAL_COMPARE.\n", __FUNCTION__);

	//比牌
	table->compare_card();

	//游戏进入比牌
	table->game_state = NORMAL_COMPARE;
...
int NormalTable::CalGroupTimeDiff()
{
	if (dealer >= 0 && dealer < MAX_CHAIR_COUNT)
	{	
...
/**
   用户上行组牌(校验牌、记录组牌时间)
*/
int NormalTable::handler_group_card(Player *player)
{
	int errCode = 0;   //错误码

	if (game_state == NORMAL_SEND_CARD)
	{
		//判断下坐位
...
bool NormalTable::isAllPlayerChoiced() 
{
	for (int i = 0; i < MAX_CHAIR_COUNT; i++)
	{
		if (seats[i].status >= STATUS_IN_GAME 
...
///"比牌"时间结束, 游戏结束，再次进入等待
void NormalTable::compare_timer_cb(struct ev_loop *loop, struct ev_timer *w, int revents)
{
	//xt_log.debug("Func[%s] *** 游戏等待 NORMAL_WAIT.\n", __FUNCTION__);
...
{	
	...
	if (NORMAL_WAIT == game_state && cur_players >= 2)
	{
		xt_log.info("Func[%s] ***  游戏准备 NORMAL_READY cur_players[%d].\n", __FUNCTION__, cur_players);
		game_state = NORMAL_READY;     
		ev_timer_again(gBull.loop, &ready_timer);
		send_game_ready_bc();
	} 
	...
	{
		game_state = NORMAL_WAIT;

		xt_log.info("Func[%s] *** 游戏等待 NORMAL_WAIT .\n", __FUNCTION__);
		Jpacket packet;
		packet.val["cmd"] = SERVER_GAME_WAIT_BC;
		packet.val["state"] = NORMAL_WAIT;
		packet.end();
...
///---处理登出
int NormalTable::handler_logout(Player *player)
{	
...
///---发聊天
int NormalTable::handler_chat(Player *player)
{
	Json::Value &val = player->client->packet.tojson();
...
///发表情
int NormalTable::handler_face(Player *player)
{
	Json::Value &val = player->client->packet.tojson();
...
///机器人定时器回调
void NormalTable::robot_timer_cb(struct ev_loop *loop, struct ev_timer *w, int revents)
{
	NormalTable *table = (NormalTable*) w->data;
...
void NormalTable::lose_exp_update(Player *player)
{
	player->incr_exp(lose_exp);
}

void NormalTable::win_exp_update(Player *player)
{
	player->incr_exp(win_exp);
}

///把单个玩家信息广播出去
int NormalTable::handler_info(Player *player)
{
	int ret = 0;
...
///---处理互动表情
int NormalTable::handler_interaction_emotion(Player *player)
{
	Json::Value &val = player->client->packet.tojson();
...
///站起，则清空坐位
void NormalTable::stand_up(Player *player)
{
...
///删除玩家
int NormalTable::del_player(Player *player)
{
...
int NormalTable::broadcast(Player *p, const std::string &packet)
{
	Player *player;
	std::map<int, Player*>::iterator it;
	for (it = playersMap.begin(); it != playersMap.end(); it++)
...
///单播
int NormalTable::unicast(Player *p, const std::string &packet)
{
...
///[starr, end]之间的随机数
int NormalTable::random(int start, int end)
{
	return start + rand() % (end - start + 1);
}

// 玩家加入游戏
int NormalTable::joinGame(Player *player)
{
	if (player != NULL)
	{
		player->join_gamed = 1;
		...
		// 加入游戏应答
		Jpacket packet;
		packet.val["cmd"] = SERVER_JOIN_GAME_UC;
		packet.val["code"] = 0;
		packet.val["msg"]="";
		packet.end();
		unicast(player, packet.tostring());
		...
		// 广播加入游戏
		Jpacket pac;
		pac.val["cmd"] = SERVER_JOIN_GAME_BC;
		pac.val["uid"] = player->uid;	
		pac.end();
		broadcast(NULL, pac.tostring());
		return 0;
	}
...
// 房主踢人
int NormalTable::kickPlayer(Player *player)
{
	Json::Value &val = player->client->packet.tojson();
	int kick_uid = val["kick_uid"].asInt();

	if (player != NULL 
		&& player->uid == create_uid)
	{
		//处理踢人
		...
		// 踢人应答
		Jpacket packet;
		packet.val["cmd"] = SERVER_KICK_PLAYER_UC;
		packet.val["code"] = 0;
		packet.val["msg"]="";
		packet.end();
		unicast(player, packet.tostring());
		...
		// 广播踢人
		Jpacket pac;
		pac.val["cmd"] = SERVER_KICK_PLAYER_BC;
        pac.val["room_id"] = tid;
        pac.val["kick_uid"] = kick_uid;
		pac.end();
		broadcast(NULL, pac.tostring());
		return 0;
	}
...
// 解散房间
int NormalTable::dismissRoom(Player *player)
{
	Json::Value &val = player->client->packet.tojson();
	int room_id = val["room_id"].asInt();
	...
	if (player != NULL)
	{
		...
		// 解散房间应答
		Jpacket packet;
		packet.val["cmd"] = SERVER_DISMISS_ROOM_UC;
		packet.val["code"] = 0;
		packet.val["msg"]="";
		packet.end();
		unicast(player, packet.tostring());
		...
		// 广播解散房间
		Jpacket pac;
		pac.val["cmd"] = SERVER_DISMISS_ROOM_BC;
		pac.val["room_id"] = room_id;
		pac.end();
		broadcast(NULL, pac.tostring());
		return 0;
	}
...
///vector转JSON数组
//void NormalTable::vector_to_json_array(std::vector<XtCard> &cards, Jpacket &packet, string key)
//{
...
//
/////map转JSON数组
//void NormalTable::map_to_json_array(std::map<int, XtCard> &cards, Jpacket &packet, string key)
//{
...
//
/////JSON数组转vector
//void NormalTable::json_array_to_vector(std::vector<XtCard> &cards, Jpacket &packet, string key)
//{
