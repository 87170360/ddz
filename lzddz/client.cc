...
_ev_nodata_tstamp(60 * 320)
{
	...
	ev_io_start(lzddz.loop, &_ev_read);
    ...
    ev_timer_start(lzddz.loop, &_ev_nodata_timer);
#if 0
    int set = 1;
...
Client::~Client()
{
	ev_io_stop(lzddz.loop, &_ev_read);
	ev_io_stop(lzddz.loop, &_ev_write);
	ev_timer_stop(lzddz.loop, &_ev_nodata_timer);
...
void Client::read_cb(struct ev_loop *loop, struct ev_io *w, int revents)
{
	...
	if (self->_state == PARSE_HEADER) {
		...
		if (self->body.length() == self->_header_p->length) {
			...
			int ret = lzddz.game->dispatch(self);
...
//    Client::pre_destroy(self);   // cfc remark and add under if by 20140124
    if (self->player) {
    	lzddz.game->del_player(self->player);
    } else {
...
int Client::send(const char *buf, unsigned int len)
{
	if (fd > 0) {
		if (_write_q.empty()) {
			_ev_write.data = this;
			ev_io_start(lzddz.loop, &_ev_write);
...
int Client::update_timer()
{
    //ev_timer_stop(lzddz.loop, &_ev_nodata_timer);
    //ev_timer_start(lzddz.loop, &_ev_nodata_timer);
    ev_timer_again(lzddz.loop, &_ev_nodata_timer);
