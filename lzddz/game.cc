...
int Game::init_table()
{
    for (int i = lzddz.conf["tables"]["begin"].asInt(); i < lzddz.conf["tables"]["end"].asInt(); i++)
    {
...
{
    xt_log.info("Listening on %s:%d\n", lzddz.conf["game"]["host"].asString().c_str(), lzddz.conf["game"]["port"].asInt());
    ...
    addr.sin_port = htons(lzddz.conf["game"]["port"].asInt());
    addr.sin_addr.s_addr = inet_addr(lzddz.conf["game"]["host"].asString().c_str());
    ...
    ev_io_start(lzddz.loop, &_ev_accept);
...
void Game::initConf(void)
{
    ...
    OUTTIME           = lzddz.conf["tables"]["outtime"].asInt();
    //xt_log.debug("OUTTIME:%d\n",OUTTIME);
    SECOND_OUTTIME    = lzddz.conf["tables"]["second_outtime"].asInt();  //第二次出牌超时
    //xt_log.debug("SECOND_OUTTIME:%d\n",SECOND_OUTTIME);
    KICKTIME          = lzddz.conf["tables"]["kicktime"].asInt();
    //xt_log.debug("KICKTIME:%d\n",KICKTIME);
    UPDATETIME        = lzddz.conf["tables"]["updatetime"].asInt();
    //xt_log.debug("UPDATETIME:%d\n",UPDATETIME);
    SHOWTIME          = lzddz.conf["tables"]["showtime"].asInt();  //发牌动画时间, 机器人根据这个延时叫分
    //xt_log.debug("SHOWTIME:%d\n",SHOWTIME);
    ROOMSCORE         = lzddz.conf["tables"]["roomscore"].asInt();  //房间底分
    //xt_log.debug("ROOMSCORE:%d\n",ROOMSCORE);
    ROOMTAX           = lzddz.conf["tables"]["roomtax"].asInt();  //房间抽水
    //xt_log.debug("ROOMTAX:%d\n",ROOMTAX);
    ALLOWANCEMONEY    = lzddz.conf["tables"]["allowancemoney"].asInt();  //破产补助
    //xt_log.debug("ALLOWANCEMONEY:%d\n",ALLOWANCEMONEY);
    MOTIONMONEY       = lzddz.conf["tables"]["motionmoney"].asInt();  //互动价格
    //xt_log.debug("MOTIONMONEY:%d\n",MOTIONMONEY);
    ROOMLIMIT         = lzddz.conf["tables"]["roomlimit"].asInt();  //房间最低携带
...
int Game::dispatch(Client *client)
{
    ...
    switch (cmd) {
        ...
        case CLIENT_GRAB:
            {
                all_tables[player->m_tid]->msgGrab(player);
            } 
            break;
        case CLIENT_DOUBLE:
            {
...
#if 1
    int i = client->uid % lzddz.main_size;
    int ret = lzddz.main_rc[i]->command(" hget u:%d skey", client->uid);
    if (ret < 0) {
        xt_log.error("check skey error, because get player infomation error.\n");
        return -1;
    }

    xt_log.debug("skey [%s] [%s]\n", client->skey.c_str(), lzddz.main_rc[i]->reply->str);
    if (lzddz.main_rc[i]->reply->str && client->skey.compare(lzddz.main_rc[i]->reply->str) != 0) {
...
int Game::add_player(Client *client)
{
    ...
    if(lzddz.cache_rc->command("hset gameinfo %d %d", lzddz.conf["game"]["port"].asInt(), fd_client.size()) < 0)
    {
...
int Game::del_player(Player *player)
{
    ...
    if(lzddz.cache_rc->command("hset gameinfo %d %d",lzddz.conf["game"]["port"].asInt(),fd_client.size()))
    {
        xt_log.error("cache redis error(set gameinfo");
    }
