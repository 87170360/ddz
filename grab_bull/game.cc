...
int Game::init_table()
{
	int vid = zjh.conf["tables"]["vid"].asInt();   //场馆id
	int zid = zjh.conf["tables"]["zid"].asInt();

	int min_money = zjh.conf["tables"]["min_money"].asInt();   //上限
	int max_money = zjh.conf["tables"]["max_money"].asInt();   //下限 

	int base_money = zjh.conf["tables"]["base_money"].asInt(); //底注
	int fee = zjh.conf["tables"]["fee"].asInt();               //每局的抽水

	int lose_exp = zjh.conf["tables"]["lose_exp"].asInt();   //经验值
	int win_exp = zjh.conf["tables"]["win_exp"].asInt();     //经验值
...
int Game::dispatch(Client *client)
{
	...
	///在线玩家数量
	if (cmd == GRAB::SYS_ONLINE) 
	{
		client->cmd_type = 1;
		Jpacket packet;
		packet.val["cmd"] = GRAB::SYS_ONLINE;
	...
	// cfc add if    ///---为机器人
	if (cmd == GRAB::CLIENT_ROBOT_REQ) 
	{
		robot_client = client;
		client->is_robot_svr = true;
		Jpacket isrobot;
		isrobot.val["cmd"] = GRAB::SERVER_ROBOT_SER_UC;
	...
	switch (cmd) {		
		case GRAB::CLIENT_GRAB_BANKER_REQ:  //抢庄
			all_tables[player->tid]->handler_grab_banker(player);
			break;

		case GRAB::CLIENT_DOUBLE_BET_REQ:   //闲家加倍下注
			all_tables[player->tid]->handler_double_bet(player);
			break;
...
int Game::login_table(Client *client, std::map<int, Table*> &a, std::map<int, Table*> &b)
{
	if (a.size() > 0)   ///要有桌子
	{
		...
		for (it = a.begin(); it != a.end(); it++) 
		{
			...
			if (player->tid == table->tid)   ///不进入之前的桌子(换桌)
			{
				continue;
			}
...
int Game::check_skey(Client *client)
{
	...
	int ret = zjh.main_rc[i]->command("hget hu:%d token", client->uid);
	if (ret < 0) {
	...
	if (zjh.main_rc[i]->reply->str 
		&& client->skey.compare(zjh.main_rc[i]->reply->str) != 0) 
	{
...
int Game::add_player(Client *client)
{
	...
	if (check_skey(client) < 0) {  
		Jpacket packet;
		packet.val["cmd"] = GRAB::SERVER_LOGIN_ERR_UC;
		packet.val["code"] = 505;
	...
	if (online_players.find(uid) != online_players.end()) {
		...
		xt_log.debug("Func[%s] player[%d] rebind by online get info ok\n",__FUNCTION__, uid);
	...
	{
		xt_log.debug("Func[%s] player[%d] rebind by offline get info ok\n", __FUNCTION__, uid);

		Player *player = offline_players[uid];

		if (all_tables.find(player->tid) == all_tables.end())
		{
			xt_log.error("Func[%s] rebind by offline uid[%d] is not in table[%d]\n", __FUNCTION__, player->uid, player->tid);
			return -1;
		}
	...
	xt_log.info("Func[%s] add new player uid[%d] login success.\n", __FUNCTION__, uid);
	return 0;
}
...
int Game::del_player(Player *player)
{
	...
	///更新gameinfo
	if(zjh.cache_rc->command("hset gameinfo %d %d",
		zjh.conf["game"]["port"].asInt(),fd_client.size()))
	{
		xt_log.error("cache redis error(set gameinfo");
	}
