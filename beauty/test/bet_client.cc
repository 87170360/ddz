	...
	public:
		virtual void onRecivePacket(XtBuffer** buffer, int num)
		{
			for(int i=0;i<num;i++)
			{
				...
				if(packet->m_val["cmd"]==BG_GAME_END_SB)
				{
				...
				int seat_id=rand()%(BG_SEAT_ID_NU);

				int bet_nu=(rand()%3+1)*2;

				if(packet->m_val["cmd"]==BG_GAME_UPDATE_SB)
				{
					if(rand()%2==0)
					{
						XtJsonPacket send_packet;
						send_packet.m_val["cmd"]=BG_PLAYER_BET_C;
						send_packet.m_val["seat_id"]=seat_id;
...
int main(int argc,char** argv)
{
	...
	if(uid==-1)
	{
		packet.m_val["cmd"]=BG_ASK_SERVER_SHUT_DOWN;
	}
	else 
	{
		packet.m_val["cmd"]=BG_LOGIN_C;
	}
