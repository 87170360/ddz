...
int main(int argc, char *argv[])
{
	...
	ret = single_instance_running(speaker.conf.get("pid_file", "conf/speaker.pid").asString().c_str());
	if (ret < 0) {
		xt_log.fatal("File: %s Func: %s Line: %d => single_instance_running.\n", __FILE__, __FUNCTION__, __LINE__);
		exit(1);
	}

	xt_log.start(speaker.conf["log"].get("log_file", "log/texas.log").asString(),
...
static void dump_conf()
{
	std::cout << "pid_file: " << speaker.conf.get("pid_file", "conf/speaker.pid").asString() << endl;
...
static int init_redis()
{
	...
	for (i = 0; i < speaker.main_size; ++i) {
		...
		ret = speaker.main_rc[i]->init(
			speaker.conf["main-db"][i]["host"].asString(),
			speaker.conf["main-db"][i]["port"].asInt(), 
			1000, 
			speaker.conf["main-db"][i]["pass"].asString());
	...
	ret = speaker.eventlog_rc->init(
		speaker.conf["eventlog-db"]["host"].asString(),
		speaker.conf["eventlog-db"]["port"].asInt(), 
		1000, 
		speaker.conf["eventlog-db"]["pass"].asString());
	...
	for (i = 0; i < speaker.sub_size; ++i) {
		...
		ret = speaker.sub_rc[i]->init(
			speaker.conf["sub-db"][i]["host"].asString(),
			speaker.conf["sub-db"][i]["port"].asInt(), 
			speaker.conf["sub-db"][i]["pass"].asString());
	...
	for (i = 0; i < speaker.pub_size; ++i) {
		...
		ret = speaker.pub_rc[i]->init(
			speaker.conf["pub-db"][i]["host"].asString(),
			speaker.conf["pub-db"][i]["port"].asInt(), 
			1000, 
			speaker.conf["pub-db"][i]["pass"].asString());
