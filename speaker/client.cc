...
void Client::read_cb(struct ev_loop *loop, struct ev_io *w, int revents)
{
	...
	if (c->_state == PARSE_HEADER) {
		...
		if (ret == 0) {
			...
			xt_log.debug("connection close in read header[%d].\n", c->fd);
			Client::destroy(c);
			return;
		}
		...
		if (c->_cur_head_len == sizeof(c->_header)) {
			if (c->_header_p->length > MAX_BUF_LEN || c->_header_p->length == 0) {
				xt_log.debug("fd[%d] receive an error length package.\n", c->fd);
				Client::destroy(c);
				return;
			}
			c->_state = PARSE_BODY;
		...
		if (ret == 0) {
			xt_log.debug("connection close in read body[%d].\n", c->fd);
			Client::destroy(c);
			return;
		}
		...
		if (c->body.length() == c->_header_p->length) {
			...
			if (speaker.server->dispatch(c) < 0){   ///---委派speaker处理
...
int Client::init_client()
{
	...
	if (ret < 0) 
	{
		xt_log.debug("client init error, because get player infomation error.\n");
		return -1;
	}
...
int Client::incr_money(int type, int value)
{
	...
	xt_log.info("incr money money old[%d] new[%d].\n", money, speaker.main_rc[index]->reply->integer);
...
{
	int ret;
	ret = commit_eventlog(uid, 0, -value, money, 201, 2);
	if (ret < 0) {
...
int Client::incr_achievement_count(int type, int value)
{
	...
	int ret = speaker.main_rc[index]->command("hincrby st:%d horn_count %d", uid, value);
	if (ret < 0) {
