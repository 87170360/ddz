...
///---整型数组
static int s_card_d[] = {0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D};//方块 A - K
static int s_card_c[] = {0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D};//梅花 A - K
static int s_card_h[] = {0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2A, 0x2B, 0x2C, 0x2D};//红桃 A - K
static int s_card_s[] = {0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x3A, 0x3B, 0x3C, 0x3D};//黑桃 A - K
///---数组指针数组
...
int XtTypeDeck::getHoleCards(XtHoleCards* card)
{
	...
	int card_type=randomType()+1;   ///---先取一个随机牌型

	int ret=getHoleCards(card,card_type);  ///---根据牌型发牌
...
int XtTypeDeck::getBaoZi(XtHoleCards* card)
{
	...
	int face=randomNum(0,XT_CARD_SINGLE_SUIT_NU);   ///---随机牌面
	int suit=randomNum(0,XT_CARD_SUIT_NU);          ///---随机花色
